	...
	for (int i = 0; i < TotalSpritesNaves; i++)
	{
		if ((Nave + i)->estado)
		{
			...
			if (i == 2 || i == 5 || i == 8 || i == 11)
			{
				...
				{
					contadorparpadeo++;
					if (contadorparpadeo > 60)
						contadorparpadeo = 0;
					if (contadorparpadeo >= 30)
						DrawColorSquare(naveposxy, c_magenta, 48, -totalcaidafuel);
					if (contadorparpadeo < 30)
						DrawColorSquare(naveposxy, c_white, 48, -totalcaidafuel);
				}
				else
				{
					DrawColorSquare(naveposxy, c_magenta, 48, -totalcaidafuel);
	...
	if (fuegonave)
	{
		for (int i = 0; i < TotalSpritesNaves; i++)
		{
			if ((Nave + i)->estado)
			{
				...
				{
					contadorfuegonave++;
					if (contadorfuegonave > 6)
						contadorfuegonave = 0;
					int hola;
					esat::Vec2 naveposxy = {504, (Nave + i)->pos.y + 72};
					DrawColorSquare(naveposxy, c_red, 48, 48, true);
					if (contadorfuegonave > 3)
						esat::DrawSprite(spritehola1, (Nave + i)->pos.x, (Nave + i)->pos.y + 72);
					if (contadorfuegonave <= 3)
						esat::DrawSprite(spritehola2, (Nave + i)->pos.x, (Nave + i)->pos.y + 72);
				}
			}
		}
	}
