...
int gravedadhola2=1,contadornave2,numerodegasofa,contadorfuegonave,contadorparpadeo;
bool empezarnivelnave=true, ocupadonave=false,bajarnave,fuegonave=false;

NaveJetpac *Nave;
esat::SpriteHandle *spritesNave;
esat::SpriteHandle spritehola1;
esat::SpriteHandle spritehola2;
...
void InicializarSpritesNave(){
	...
	spritehola1=esat::SubSprite(spritefinal,864,0,48,48);
	spritehola2=esat::SubSprite(spritefinal,912,0,48,48);
}
...
for(int i=0;i<TotalSpritesNaves;i++) {
    	if((Nave+i)->estado){
				...
				if(i==2 || i==5 || i==8 || i==11){
					...
					if(totalcaidafuel==144){
						contadorparpadeo++;
						if(contadorparpadeo>60)contadorparpadeo=0;
						if(contadorparpadeo>=30)DrawColorSquare(naveposxy,c_magenta,48,-totalcaidafuel);
						if(contadorparpadeo<30)DrawColorSquare(naveposxy,c_white,48,-totalcaidafuel);
					}else{
	...
	if(fuegonave){
    	...
    	if((Nave+i)->estado){
				if(i==2 || i==5 || i==8 || i==11){
					contadorfuegonave++;
					if(contadorfuegonave>6)contadorfuegonave=0;
					int hola;
					esat::Vec2 naveposxy={504,(Nave+i)->pos.y+72};
					DrawColorSquare(naveposxy,c_red,48,48,true);
					if(contadorfuegonave>3)esat::DrawSprite(spritehola1,(Nave+i)->pos.x,(Nave+i)->pos.y+72);
					if(contadorfuegonave<=3)esat::DrawSprite(spritehola2,(Nave+i)->pos.x,(Nave+i)->pos.y+72);
				}
		}
	}
	}
